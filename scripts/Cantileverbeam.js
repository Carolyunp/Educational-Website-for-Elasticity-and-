...
function calculateStress(x, y) {
    ...
    if (currentCase.includes("M0")) {
        A4 = -q*h*h/(120*I) - M0/(6*I);
    } else {
        A4 = -q*h*h/(120*I);
    }
    
    if (currentCase.includes("T0")) {
        A5 = T0/(2*h);
    } else {
        A5 = 0;
    }
...
function calculateDisplacement(x, y) {
    ...
    const u = (1/(E*I))*(
        ...
        I*(ν*q/2 + 2*A5)*(x - l) - 
        (1/(4*(1 + β)*E*I))*(1 + ν)*(q*l + (currentCase.includes("F0") ? F0 : 0))*h*h*y
    );
    ...
    const v = (1/(E*I))*(
        ...
        I*(q/2 + 2*ν*A5)*y - 
        (β/(4*(1 + β)*E*I))*(1 + ν)*(q*l + (currentCase.includes("F0") ? F0 : 0))*(x - l)*h*h
    );
